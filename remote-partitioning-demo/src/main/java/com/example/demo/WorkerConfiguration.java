 ...
 * "requests" channel. For each incoming StepExecutionRequest it: 1. reloads the
 * StepExecution from the shared JobRepository (by id), 2. calls
 * workerStep.execute(stepExecution) <-- execute() + the lock map live HERE, 3. sends the
 * resulting StepExecution back on the "replies" channel.
 */
@Configuration
@Profile("worker")
public class WorkerConfiguration {
	...
	public IntegrationFlow outboundReplies(RabbitTemplate rabbitTemplate) {
		return IntegrationFlow.from(replies()).handle(Amqp.outboundAdapter(rabbitTemplate).routingKey("replies")).get();
	}
