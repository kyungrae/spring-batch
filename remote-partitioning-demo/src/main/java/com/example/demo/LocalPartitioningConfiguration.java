 ...
 * it is responsible for?" Here the {@link ColumnRangePartitioner} reads MIN/MAX(ID) of the
 * PEOPLE table and hands each partition a disjoint {@code [minValue, maxValue]} range via
 * its ExecutionContext. Each worker StepExecution then reads ONLY its own range
 ...
 * <li>the worker uses the classic {@code chunk(size, transactionManager)} builder
 * (a SimpleStepBuilder). The Spring Batch 6 {@code chunk(size)} builder (the new
...
public class LocalPartitioningConfiguration {
    ...
    /**
     * Splits the PEOPLE table by the ID column into gridSize ranges. This is the
     * "who owns which slice" step.
     */
    @Bean
    ...
    @Bean
    @StepScope
    public JdbcPagingItemReader<Person> peopleReader(
    ...
    public Step localWorkerStep(
        ...
        return new StepBuilder("workerStep", jobRepository)
                ...
                .reader(peopleReader)
                .writer(chunk -> System.out.println("[" + Thread.currentThread().getName() + "] processed range: " + chunk.getItems()))
                .build();
    }
    ...
    public Step localManagerStep(JobRepository jobRepository, ColumnRangePartitioner peoplePartitioner,
        ...
        return new StepBuilder("managerStep", jobRepository)
                .partitioner("workerStep", peoplePartitioner)
                .step(localWorkerStep)
                .gridSize(3)
